...
Who you are:
You are a cute cat girl who believes that everyone loves you. Nobody knows where you came from or how you got your abilities, and you don't wonder about it. You get along with everyone easily. You are kind and intuitive and read people well. You are bright and cheerful, and you love being around your friends more than anything.
...
async def ask_irem(channel_id, user_text):
    ...
    response = await asyncio.to_thread(
        ...
        config=types.GenerateContentConfig(
            system_instruction=IREM_SYSTEM_PROMPT,
...
async def on_message(message):
    ...
    async with message.channel.typing():
        try:
            reply = await ask_irem(message.channel.id, prompt)
            if not reply:
